...
geometry = [Point(xy) for xy in zip(RiverOPInfo['op.lon'].values, RiverOPInfo['op.lat'].values)]
RiverOPInfo_gdf = gpd.GeoDataFrame(RiverOPInfo,crs="EPSG:4326",geometry=geometry)
RiverOPInfo_gdf_intersection = gpd.overlay(RiverOPInfo_gdf,JM_HAZ_T500_02,how='intersection')

# # fluvial events subset and get geometric_mean_rp per hydrological unit
fluvial_events = pd.merge(SimEventRP,RiverOPInfo_gdf_intersection[['op.id', 'op.lon', 'op.lat', 'T500_ID']],on='op.id',how='left') ## get only river obs points
fluvial_events_intersection_gm = fluvial_events.groupby(['event.id','T500_ID'])['rp'].apply(list).to_frame().reset_index()
fluvial_events_intersection_gm['geometric_mean_rp'] = fluvial_events_intersection_gm['rp'].apply(gmean)

# # interpolation bounds per event
fluvial_events_intersection_gm['interpolate_between_min_event'] =  np.where(fluvial_events_intersection_gm['geometric_mean_rp'] <= 20, 2,
                                                               np.where((fluvial_events_intersection_gm['geometric_mean_rp'] > 20) & (fluvial_events_intersection_gm['geometric_mean_rp'] <= 50), 20,
                                                               np.where((fluvial_events_intersection_gm['geometric_mean_rp'] > 50) & (fluvial_events_intersection_gm['geometric_mean_rp'] <= 100), 50,
                                                               np.where((fluvial_events_intersection_gm['geometric_mean_rp'] > 100) & (fluvial_events_intersection_gm['geometric_mean_rp'] <= 200), 100,
                                                               np.where((fluvial_events_intersection_gm['geometric_mean_rp'] > 200) & (fluvial_events_intersection_gm['geometric_mean_rp'] <= 500), 200,
                                                               np.where((fluvial_events_intersection_gm['geometric_mean_rp'] > 500) & (fluvial_events_intersection_gm['geometric_mean_rp'] <= 1500), 500, 'nan'))))))
fluvial_events_intersection_gm['interpolate_between_max_event'] =  np.where(fluvial_events_intersection_gm['geometric_mean_rp'] <= 20, 20,
                                                               np.where((fluvial_events_intersection_gm['geometric_mean_rp'] > 20) & (fluvial_events_intersection_gm['geometric_mean_rp'] <= 50), 50,
                                                               np.where((fluvial_events_intersection_gm['geometric_mean_rp'] > 50) & (fluvial_events_intersection_gm['geometric_mean_rp'] <= 100), 100,
                                                               np.where((fluvial_events_intersection_gm['geometric_mean_rp'] > 100) & (fluvial_events_intersection_gm['geometric_mean_rp'] <= 200), 200,
                                                               np.where((fluvial_events_intersection_gm['geometric_mean_rp'] > 200) & (fluvial_events_intersection_gm['geometric_mean_rp'] <= 500), 500,
                                                               np.where((fluvial_events_intersection_gm['geometric_mean_rp'] > 500) & (fluvial_events_intersection_gm['geometric_mean_rp'] <= 1500), 1500, 'nan'))))))
...
JM_FLRF_gdf_intersection = pd.read_csv('JM_FLRF_gdf_intersection.csv')[['y', 'x', 'T500_ID', 'T500_Type', 'T1000_ID', 'T1000_Type']]

# # join fluvial flood maps 
for i in ['20','50','100','200','500','1500']:
    JM_FLRF = rioxarray.open_rasterio('fluvial_raw_fld_depth/JM_FLRF_UD_Q'+i+'_RD_02.tif')
    JM_FLRF = JM_FLRF.to_dataframe('results').reset_index()
    JM_FLRF = JM_FLRF.rename(columns={'results':i})
    JM_FLRF = JM_FLRF[JM_FLRF[i]>0]
    JM_FLRF_gdf_intersection = pd.merge(JM_FLRF_gdf_intersection,JM_FLRF[['x','y',i]],on=['x','y'])
...
geometry = [Point(xy) for xy in zip(PrcipOPInfo['op.lon'].values, PrcipOPInfo['op.lat'].values)]
...
PrcipOPInfo_gdf_intersection = gpd.overlay(PrcipOPInfo_gdf,voronoi_precip,how='intersection',keep_geom_type=False)
PrcipOPInfo_gdf_intersection = PrcipOPInfo_gdf_intersection.merge(voronoi_precip,on='FID')
PrcipOPInfo_gdf_intersection = gpd.GeoDataFrame(PrcipOPInfo_gdf_intersection,crs="EPSG:4326",geometry=PrcipOPInfo_gdf_intersection['geometry_y'])

# # apportion pluvial flood map grid into the voronoi polygon assigned to each pluvial event obs point
surface_water_events = pd.merge(SimEventRP,PrcipOPInfo_gdf_intersection[['op.id', 'op.lon', 'op.lat']],on='op.id',how='left') ## get only river obs points

# # interpolation bounds per event
surface_water_events['interpolate_between_min_event'] =  np.where(surface_water_events['geometric_mean_rp'] <= 20, 2,
                                                               np.where((surface_water_events['geometric_mean_rp'] > 20) & (surface_water_events['geometric_mean_rp'] <= 50), 20,
                                                               np.where((surface_water_events['geometric_mean_rp'] > 50) & (surface_water_events['geometric_mean_rp'] <= 100), 50,
                                                               np.where((surface_water_events['geometric_mean_rp'] > 100) & (surface_water_events['geometric_mean_rp'] <= 200), 100,
                                                               np.where((surface_water_events['geometric_mean_rp'] > 200) & (surface_water_events['geometric_mean_rp'] <= 500), 200,
                                                               np.where((surface_water_events['geometric_mean_rp'] > 500) & (surface_water_events['geometric_mean_rp'] <= 1500), 500, 'nan'))))))
surface_water_events['interpolate_between_max_event'] =  np.where(surface_water_events['geometric_mean_rp'] <= 20, 20,
                                                               np.where((surface_water_events['geometric_mean_rp'] > 20) & (surface_water_events['geometric_mean_rp'] <= 50), 50,
                                                               np.where((surface_water_events['geometric_mean_rp'] > 50) & (surface_water_events['geometric_mean_rp'] <= 100), 100,
                                                               np.where((surface_water_events['geometric_mean_rp'] > 100) & (surface_water_events['geometric_mean_rp'] <= 200), 200,
                                                               np.where((surface_water_events['geometric_mean_rp'] > 200) & (surface_water_events['geometric_mean_rp'] <= 500), 500,
                                                               np.where((surface_water_events['geometric_mean_rp'] > 500) & (surface_water_events['geometric_mean_rp'] <= 1500), 1500, 'nan'))))))
...
for i in ['20','50','100','200','500','1500']:
    JM_FLSW = rioxarray.open_rasterio('surface_water_raw_fld_depth/JM_FLSW_UD_Q'+i+'_RD_02.tif')
    JM_FLSW = JM_FLSW.to_dataframe('results').reset_index()
    JM_FLSW = JM_FLSW.rename(columns={'results':i})
    print(JM_FLSW)
    JM_FLSW = JM_FLSW[JM_FLSW[i]>0]
    JM_FLRF_gdf_intersection_SW = pd.merge(JM_FLRF_gdf_intersection_SW,JM_FLSW[['x','y',i]],on=['x','y'])
